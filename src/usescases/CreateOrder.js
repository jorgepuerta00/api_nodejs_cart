...
class CreateCart {
    ...
    execute() {
        ...
        var saveCart = this.saveCart();
        this.sendEmailConfirmationCart();

        return saveCart;
    }

    saveCart() {
        return this.cartRepository.save(this.cart);
    }

    setCartId() {
        this.generateUUID.setBytes(16);
        this.cart.id = this.generateUUID.execute();
    }

    setCartState() {
        this.cart.state = "active";
    }

    setCartCode() {
        this.generateUUID.setBytes(8);
        this.cart.code = this.generateUUID.execute();
    }

    setCartDateTime() {
        this.cart.datetime = moment().format('YYYYMMDDHHmmss');
    }

    sendEmailConfirmationCart() {
    ...
    createEmailBodyFromCart() {
        ...
        this.cart.products.forEach(element => {
            ...
            htmlProductTemp = htmlProductTemp.replace('@datetime', this.cart.datetime);

            htmlProducts = htmlProducts.concat(htmlProductTemp);
        });

        htmlBody = htmlBody.replace('@code', this.cart.code);
        htmlBody = htmlBody.replace('@datetime', this.cart.datetime);
